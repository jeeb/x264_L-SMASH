...
generic_option:
    ...
    if( ( ( info.csp & X264_CSP_MASK ) >= X264_CSP_BGR && ( output_csp & X264_CSP_MASK ) <= X264_CSP_YV24 ) && !b_user_colormatrix )
        info.colormatrix = 5; /* bt470bg ; swscale and avisynth by default do their magic in it. */
    else if( ( ( info.csp & X264_CSP_MASK ) <= X264_CSP_YV24 && ( output_csp & X264_CSP_MASK ) >= X264_CSP_BGR ) && !b_user_colormatrix )
